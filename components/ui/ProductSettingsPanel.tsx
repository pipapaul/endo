...
interface ProductSettingsPanelProps {
  ...
  /** Called when user should navigate to product configuration page */
...
export const ProductSettingsPanel: React.FC<ProductSettingsPanelProps> = ({
  ...
  // State for prompting user to configure products after selecting extended PBAC
  ...
  const handleTrackingMethodChange = (method: TrackingMethod) => {
    ...
    if (todayHasBleedingData) {
      ...
      // Prompt to configure products when switching to extended mode
      if (method === "pbac_extended" && onNavigateToProducts) {
  ...
  const handleModeChangeConfirm = () => {
    if (pendingMethodChange) {
      ...
      // Prompt to configure products when switching to extended mode
      if (pendingMethodChange === "pbac_extended" && onNavigateToProducts) {
        setShowProductPrompt(true);
      }
    }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Product Configuration Prompt Dialog */}
      {showProductPrompt && onNavigateToProducts && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[110]">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md mx-4 shadow-xl">
            <h3 className="text-lg font-semibold text-rose-900 mb-3">
            ...
            <p className="text-sm text-rose-700 mb-6">
              Für den erweiterten PBAC-Modus kannst du deine Menstruationsprodukte konfigurieren – z.B. Tassen, Discs oder Periodenslips mit ihren Kapazitäten.
            </p>
            <div className="flex gap-3">
              <Button
                type="button"
...
/**
 * Standalone product configuration panel for the products settings page.
 */
...
export const ProductConfigPanel: React.FC<ProductConfigPanelProps> = ({
  settings,
  onSettingsChange,
}) => {
  const [showAddCustom, setShowAddCustom] = useState(false);
  ...
  // Filter products for extended PBAC mode (no classic PBAC products)
  const allProducts = [...DEFAULT_PRODUCTS, ...settings.customProducts].filter(
    (product) => product.isClassicPbac !== true
  );
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-rose-600">
        Wähle die Produkte aus, die du verwenden möchtest. Du kannst auch eigene Produkte mit individuellen Kapazitäten hinzufügen.
      </p>
